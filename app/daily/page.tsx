...
type PairedEntry = {
  date: string;
  question: string;
  my: { body: string; emoji: string | null };
...
export default async function DailyHistoryPage() {
  ...
  const pairedDates = [...byDate.entries()]
    .filter(([, v]) => v.me && v.partner)
    .map(([d]) => d)
    .sort((a, b) => b.localeCompare(a));

  const questions = await prisma.dailyQuestion.findMany({
    where: { date: { in: pairedDates } },
  });
  const qMap = new Map(questions.map((q) => [q.date, q.question]));

  const paired: PairedEntry[] = pairedDates.map((d) => {
